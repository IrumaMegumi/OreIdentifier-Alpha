...
void OreIdentifier::_ImgPreprocess() {
	...
	*/
	...
	cvtColor(_img, _img, COLOR_BGR2GRAY);
	medianBlur(_img, _img, 3);//中值滤波
	adaptiveThreshold(_img, _img, 255, ADAPTIVE_THRESH_MEAN_C, THRESH_BINARY_INV, 9.5, 10);//自适应阈值分离


	Mat kernel = getStructuringElement(MORPH_RECT, Size(2, 2));
	erode(_img, _img, kernel);//腐蚀

	//imshow("img_gray", img_gray);
	//imshow("img_thres", img_thres);
	imshow("_img", img_erode);
}
